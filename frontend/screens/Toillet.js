...
const Toillet = ({navigation}) => {
  ...
  const etatDePorteToillet = () => {
    setPortouvert(!portouvert)
  }

  const etatDeLumiereToillet = () => {
    setLumiereallumer(!lumiereallumer)
  }

  const etatDeFenetreToillet = () => {
    setFenetreouvert(!fenetreouvert)
  }

  const etatDeRideauToillet  = () => {
