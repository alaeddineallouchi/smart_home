...
const Jardin = ({navigation}) => {
  ...
  const etatDePorteJardin = () => {
    setPortouvert(!portouvert)
  }

  const etatDeLumiereJardin = () => {
    setLumiereallumer(!lumiereallumer);
  }

  const etatDeFenetreJardin = () => {
    setFenetreouvert(!fenetreouvert);
  }

  const etatDeRideauJardin  = () => {
