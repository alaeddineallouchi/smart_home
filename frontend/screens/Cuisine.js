...
const Cuisine = ({navigation}) => {
  ...
  const etatDePorteChambre = () => {
    setPortouvert(!portouvert)
  }

  const etatDeLumiereChambre = () => {
    setLumiereallumer(!lumiereallumer)
  }

  const etatDeFenetreChambre = () => {
    setFenetreouvert(!fenetreouvert)
  }

  const etatDeRideauChambre  = () => {
