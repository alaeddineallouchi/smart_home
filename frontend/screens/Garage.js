...
const Garage = ({navigation}) => {
  ...
  const etatDePorteGarage = () => {
    setPortouvert(!portouvert)
  }

  const etatDeLumiereGarage = () => {
    setLumiereallumer(!lumiereallumer)
  }

  const etatDeFenetreGarage = () => {
    setFenetreouvert(!fenetreouvert)
  }

  const etatDeRideauGarage  = () => {
